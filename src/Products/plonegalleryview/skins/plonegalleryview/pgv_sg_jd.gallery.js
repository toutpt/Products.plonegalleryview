...
var gallery = new Class({
	...
	initCarousel: function () {
		...
		this.carouselInner.style.width = ((this.maxIter * (this.options.thumbWidth + this.options.thumbSpacing + 2)) - this.options.thumbSpacing + this.options.thumbWidth) + "px";
	},
		...
		for(i=0;i<this.galleryData.length;i++)
		{
			...
			currentImg.element.addEvents({
				...
				'mouseout': function (myself) {
					myself.clearTimer();
					myself.custom(this.options.thumbIdleOpacity);
	...
	clearThumbnailsHighlights: function()
	{
		for(i=0;i<this.galleryData.length;i++)
		{
			...
			this.thumbnailElements[i].custom(this.options.thumbIdleOpacity);
		}
	},
	centerCarouselOn: function(num) {
        ...
        this.carouselWrapper.onmouseover = MouseOverStatusOn;
        this.carouselWrapper.onmouseout = MouseOverStatusOff;	
        	
        function MouseOverStatusOn() {
            MouseOverStatus = 1;
        }
        function MouseOverStatusOff() {
            MouseOverStatus = 0;
        }
        
        if (MouseOverStatus == 0) {
            this.clearThumbnailsHighlights();
